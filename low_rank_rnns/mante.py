...
def generate_mante_data(num_trials, coherences=None, std=std_default, fraction_validation_trials=0.,
    ...
    for i in range(num_trials):
        if random.random() > fraction_catch_trials:
            ...
            if context == 1:
                inputs[i, fixation_duration_discrete:response_begin, 2] = 1.
                targets[i, response_begin:] = hi if coh_color > 0 else lo
            elif context == 2:
                inputs[i, fixation_duration_discrete:response_begin, 3] = 1.
    ...
    if fraction_validation_trials > 0:
        return inputs_train, targets_train, mask_train, inputs_val, targets_val, mask_val, epochs
    elif fraction_validation_trials == 0.:
        return inputs_train, targets_train, mask_train, epochs
    ...
    for ctx in (0, 1):
        ...
        for i, coh1 in enumerate(coherences):
            for j, coh2 in enumerate(coherences):
                ...
                inputs[:, fixation_duration_discrete:response_begin, 2+ctx] = 1.
