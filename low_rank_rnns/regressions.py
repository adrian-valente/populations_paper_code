...
def regression_rdm(net, samples_per_coh=10, cohs=(-1, 0, 1)):
    # Building the inputs
    inputs = rdm.std_default * torch.randn((samples_per_coh * len(cohs), rdm.total_duration, 1))
    for i in range(len(cohs)):
        for j in range(samples_per_coh):
            inputs[i * samples_per_coh + j, rdm.fixation_duration_discrete:rdm.stimulus_end, :] += cohs[i] * rdm.SCALE
    ...
    target1 = np.mean(rates[:, rdm.fixation_duration_discrete:rdm.fixation_duration_discrete + 2, :], axis=1)
    reg1 = LinearRegression(fit_intercept=False).fit(coherence_trials.reshape((-1, 1)), target1)
...
def regression_romo(net, nsamples=100):
    ...
    response_begin = int(romo.stim1_end + delay_duration + romo.stimulus2_duration_discrete)
...
def regression_raposo(net, cohs=(-4, -2, -1, 1, 2, 4)):
        ...
        for i, coh1 in enumerate(cohs):
            for j, coh2 in enumerate(cohs):
                ...
                if ctx in (1, 0):
                    ...
                    inputs[trial_idx, raposo.stim_begin:raposo.stim_end, 2] = raposo.SCALE_CTX
                    cohs1.append(coh1)
                else:
                    cohs1.append(0)
                if ctx in (-1, 0):
                    inputs[trial_idx, raposo.stim_begin:raposo.stim_end, 1] = coh2 * raposo.SCALE
                    inputs[trial_idx, raposo.stim_begin:raposo.stim_end, 3] = raposo.SCALE_CTX
                    cohs2.append(coh2)
                else:
                    cohs2.append(0)
    ctx1 = [0] * lc2 + [1] * (2 * lc2)
    ...
    predictors = np.stack([np.array(cohs1), np.array(cohs2), np.array(ctx1), np.array(ctx2)]).T
...
def regression_mante(net):
    ...
    for ctx in (-1, 0, 1):
        for i, coh1 in enumerate(cohs):
            for j, coh2 in enumerate(cohs):
                inputs[trial_idx, mante.stim_begin:mante.stim_end, 0] += coh1 * mante.SCALE
                inputs[trial_idx, mante.stim_begin:mante.stim_end, 1] += coh2 * mante.SCALE
                if ctx == 1:
                    inputs[trial_idx, mante.fixation_duration_discrete:, 2] = 1 * mante.SCALE_CTX
                    predictors[trial_idx, 2] = 1
                elif ctx == -1:
                    inputs[trial_idx, mante.fixation_duration_discrete:, 3] = 1 * mante.SCALE_CTX
