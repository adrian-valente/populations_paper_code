...
def neighbors_distance_distr(X, n_neighbors=1):
    n = X.shape[0]
    nn = NearestNeighbors(
        n_neighbors=n_neighbors + 1, algorithm="brute", metric="cosine"
    )
    nn.fit(X)
    distr = np.empty(n)
    for i in range(n):
        xi = X[i]
        ind = nn.kneighbors(
            xi.reshape(1, -1), return_distance=False
...
def epairs(
    ...
    if metric == "angles":
        angles_data = np.arccos(angles_data.clip(-1.0, 1.0))
