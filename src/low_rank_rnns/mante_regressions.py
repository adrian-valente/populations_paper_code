...
def compute_sensory_regressors(net, epoch_start, epoch_end):
    ...
    for context in (1, 2):
        ...
        inputs_array[0] = mante.generate_specified_input(0, 0, context, 1, 0.0)
        ...
        for i in range(len(coh1_vect)):
            for j in range(len(coh2_vect)):
                coh1 = coh1_vect[i]
                coh2 = coh2_vect[j]
                input_trial = mante.generate_specified_input(
                    coh1, coh2, context, 1, 0.0
                )
        ...
        variables_to_be_regressed = torch.mean(
            trajectories[:, stimulation_epoch, :], dim=1
        )
        variables_to_be_regressed = variables_to_be_regressed.detach().numpy()
        X = np.concatenate(
            (
                color_vector.reshape(len(color_vector), 1),
                motion_vector.reshape(len(color_vector), 1),
            ),
            axis=1,
        )
        print(X.shape)
...
def compute_sensory_regressors_mixed_ctx(
    net, epoch_start, epoch_end, data_type="mante_expe"
):
    context = 1
    coh1_vect = np.array([-16, -8, -4, -2, -1, 0, 1, 2, 4, 8, 16])
    coh2_vect = np.array([-16, -8, -4, -2, -1, 0, 1, 2, 4, 8, 16])
    input_trial, y_train, mask_train, epochs = mante.generate_mante_data(
        1, coh_color_spec=0, coh_motion_spec=0, context_spec=context, std=0
    )
...
def compute_sensory_context_regressors(
    net, epoch_start, epoch_end, data_type="mante_expe"
):
    context = 1
    coh1_vect = np.array([-16, -8, -4, -2, -1, 0, 1, 2, 4, 8, 16])
    ...
    X = np.concatenate(
        (
            color_vector.reshape(len(color_vector), 1),
            motion_vector.reshape(len(color_vector), 1),
            context_vector.reshape(len(context_vector), 1),
        ),
        axis=1,
    )
    print(X.shape)
...
def compute_choice_sensory_regressors(net, rates=True):
    ...
    for ctx in (0, 1):
        for i, coh1 in enumerate(cohs):
            for j, coh2 in enumerate(cohs):
                x, y, mask, epochs = mante.generate_mante_data(
                    1,
                    std=0,
    ...
    # choice
    predictors_choice = np.sign(
        np.mean(output[:, mante.response_begin :], axis=1)
    ).reshape((-1, 1))
...
def compute_choice_regressors(
    net, epoch_start, epoch_end, ctx1=1, ctx2=2, data_type="mante_expe", nb_regressors=1
):
    ...
    if nb_regressors == 1:
        ...
        X = np.concatenate(
            (
                choice_vector1.reshape(len(choice_vector1), 1),
                choice_vector2.reshape(len(choice_vector2), 1),
            ),
            axis=1,
        )
...
def compute_choice_regressors_split_context(
    net, epoch_start, epoch_end, ctx1=1, ctx2=2, data_type="mante_expe", nb_regressors=1
):
    # NB: we here assume that the network performs perfectly (choice is not the actual choice of the net but the correct choice)
    context = 1
...
def compute_choice_sensory_regressors_split_ctx(
    net, epoch_start, epoch_end, data_type="mante_expe"
):
    context = +1
    ...
    X = np.concatenate(
        (
            color_vector.reshape(len(color_vector), 1),
            motion_vector.reshape(len(color_vector), 1),
            choice_vector.reshape(len(choice_vector), 1),
        ),
        axis=1,
    )
    print(X.shape)
    ...
    context = 2
    input_trial, y_train, mask_train, epochs = mante.generate_mante_data(
        1, coh_color_spec=coh1, coh_motion_spec=coh2, context_spec=context, std=0
    )
    inputs_array = 0 * input_trial
    color_vector = np.array([])
    motion_vector = np.array([])
...
def compute_ctx_regressors(net):
    ...
    input_trial, y_train, mask_train, epochs = mante.generate_mante_data(
        1, coh_color_spec=0, coh_motion_spec=0, context_spec=+1, std=0
    )
    inputs_array = 0 * input_trial
    ...
    X = np.concatenate(
        (
            context1_vector.reshape(len(context1_vector), 1),
            context2_vector.reshape(len(context2_vector), 1),
        ),
        axis=1,
    ).T
    X = X.T
...
def compute_single_ctx_regressors(net):
    ...
    input_trial = mante.generate_specified_input(0, 0, 1, 1, 0.0)
    inputs[0] = input_trial
    input_trial = mante.generate_specified_input(0, 0, 2, 1, 0.0)
...
def plot_tuning_curves(
    net, ind_neuron, epoch_start, epoch_end, add_linreg=False, col1="m", col2="c"
):
    """
    ...
    for i in range(len(coh_vect)):
        ...
        coh2 = 0
        context = 1
        input1 = mante.generate_specified_input(coh1, coh2, context, 1, 0.0)
        z, x = net.forward(input1, return_dynamics=True)
        ra = float(
            np.mean(np.tanh(x[0, epoch_start:epoch_end, ind_neuron].detach().numpy()))
        )
    ...
    plt.plot(
        coh_vect,
        (color_selectivity_ctx1 - color_selectivity_ctx1[0])
        * np.sign(color_selectivity_ctx1[-1] - color_selectivity_ctx1[0]),
        "s-",
        c=col1,
    )
    plt.plot(
        coh_vect,
        (color_selectivity_ctx2 - color_selectivity_ctx2[0])
        * np.sign(color_selectivity_ctx2[-1] - color_selectivity_ctx2[0]),
        "s-",
        c=col2,
    )
    plt.figure(1)
    plt.plot(
        coh_vect,
        (motion_selectivity_ctx1 - motion_selectivity_ctx1[0])
        * np.sign(motion_selectivity_ctx1[-1] - motion_selectivity_ctx1[0]),
        "s-",
        c=col1,
    )
    plt.plot(
        coh_vect,
        (motion_selectivity_ctx2 - motion_selectivity_ctx2[0])
        * np.sign(motion_selectivity_ctx2[-1] - motion_selectivity_ctx2[0]),
        "s-",
        c=col2,
    )

    if add_linreg:
        vtbr = (color_selectivity_ctx1 - color_selectivity_ctx1[0]) * np.sign(
            color_selectivity_ctx1[-1] - color_selectivity_ctx1[0]
        )
        ...
        vtbr = (color_selectivity_ctx2 - color_selectivity_ctx2[0]) * np.sign(
            color_selectivity_ctx2[-1] - color_selectivity_ctx2[0]
        )
        X = coh_vect.reshape((len(coh_vect), 1))
        vtbr_reshaped = vtbr.reshape(len(vtbr), 1)
        reg = LinearRegression().fit(X, vtbr_reshaped)
        ...
        vtbr = (motion_selectivity_ctx1 - motion_selectivity_ctx1[0]) * np.sign(
            motion_selectivity_ctx1[-1] - motion_selectivity_ctx1[0]
        )
        X = coh_vect.reshape((len(coh_vect), 1))
        vtbr_reshaped = vtbr.reshape(len(vtbr), 1)
        reg = LinearRegression().fit(X, vtbr_reshaped)
        ...
        vtbr = (motion_selectivity_ctx2 - motion_selectivity_ctx2[0]) * np.sign(
            motion_selectivity_ctx2[-1] - motion_selectivity_ctx2[0]
        )
        X = coh_vect.reshape((len(coh_vect), 1))
        vtbr_reshaped = vtbr.reshape(len(vtbr), 1)
        reg = LinearRegression().fit(X, vtbr_reshaped)
    ...
    return (
        color_selectivity_ctx1,
        motion_selectivity_ctx1,
        color_selectivity_ctx2,
        motion_selectivity_ctx2,
    )
