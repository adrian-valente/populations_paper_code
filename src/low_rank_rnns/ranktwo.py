...
def plot_field(net, vec1=None, vec2=None, xmin=-3, xmax=3, ymin=-3, ymax=3, input=None, res=50,
               ...
               orth=False, alt_naming=True, sizes=1):
    """
    ...
    if add_fixed_points:
        if fp_load is None:
            ...
            for i in range(X_grid.size):
                ...
                # if solution found
                if sol.success == 1:
                    kappa_sol = [(sol.x @ vec1) / sqrt(net.hidden_size), (sol.x @ vec2) / sqrt(net.hidden_size)]
                    # Computing stability
...
def plot_field_noscalings(net, vec1=None, vec2=None, xmin=-3, xmax=3, ymin=-3, ymax=3, input=None, res=50,
               ax=None, add_fixed_points=False, fixed_points_trials=10, fp_save=None, fp_load=None, nojac=False,
               orth=False, sizes=1.):
    """
...
def plot_readout_map(vec1, vec2, wo, rect=(-10, 10, -10, 10), scale=.5, scalings=True, cmap='jet'):
    """
