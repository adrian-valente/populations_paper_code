...
for i in range(n_nets):
    ...
    dms.setup()
    x_train, y_train, mask_train, x_val, y_val, mask_val = dms.generate_dms_data(1000)
    train(net, x_train, y_train, mask_train, lr=1e-2, n_epochs=300, early_stop=0.05, keep_best=True, cuda=True, clip_gradient=.01)
    ...
    dms.setup()
    x_train, y_train, mask_train, x_val, y_val, mask_val = dms.generate_dms_data(1000)
    train(net, x_train, y_train, mask_train, lr=1e-4, n_epochs=400, early_stop=0.05, keep_best=True, cuda=True)
...
np.savez('../data/dms_results3.npz', p_vals, c_vals, preg_vals, creg_vals, acc_vals, loss_vals,
         accs_res, losses_res)
