...
for rank in (2, 3, 4):
    print(f"rank = {rank}")
    x_train, y_train, mask_train, x_val, y_val, mask_val, epochs = mante.generate_mante_data(1000, fraction_validation_trials=.2)
    net = LowRankRNN(4, hidden_size, 1, noise_std, alpha, rank=rank, train_wi=True, train_wo=True)
    train(net, x_train, y_train, mask_train, lr=lr_base / sqrt(hidden_size), n_epochs=70, keep_best=True, cuda=True,
          clip_gradient=.1)
    ...
    train(net2, x_train, y_train, mask_train, 100, lr=1e-4, resample=True, cuda=True, keep_best=True)
    for _ in range(n_samples):
        net2.resample_basis()
        loss, acc = mante.test_mante(net2, x_val, y_val, mask_val)
        losses_normal.append(loss)
        accs_normal.append(acc)
    print(f"gaussian resampling: min={np.min(accs_normal):.2f}, med={np.median(accs_normal):.2f},"
    ...
    train(net3, x_train, y_train, mask_train, 100, lr=1e-4, resample=True, cuda=True, keep_best=True)
    for _ in range(n_samples):
        net3.resample_basis()
        loss, acc = mante.test_mante(net3, x_val, y_val, mask_val)
        losses2.append(loss)
        accs2.append(acc)
    print(f"2 populations resampling: min={np.min(accs2):.2f}, med={np.median(accs2):.2f},"
