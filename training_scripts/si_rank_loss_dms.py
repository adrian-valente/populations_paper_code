...
train(net_fr, x_train0, y_train0, mask_train0, n_epochs=n_epochs // 4, lr=1e-4, keep_best=True, cuda=True)
train(net_fr, x_train1, y_train1, mask_train1, n_epochs * 2, lr=1e-5, keep_best=True, cuda=True)
...
m_init = torch.from_numpy(s[:5] * u[:, :5]).to(device=net_fr.wrec.device) * sqrt(hidden_size)
n_init = torch.from_numpy(v[:5, :].transpose()).to(device=net_fr.wrec.device) * sqrt(hidden_size)

for rank in range(5, 0, -1):
    net = LowRankRNN(2, hidden_size, 1, noise_std, alpha, rank=rank,
                                       wi_init=wi_init, n_init=n_init[:, :rank],
                                       m_init=m_init[:, :rank], wo_init=wo_init)
    train(net, x_train0, y_train0, mask_train0, lr=1e-2, n_epochs=n_epochs // 4, keep_best=True, cuda=True)
    train(net, x_train1, y_train1, mask_train1, lr=1e-2, n_epochs=n_epochs, keep_best=True, cuda=True)
