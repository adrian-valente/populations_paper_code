...
net = SupportLowRankRNN(2, hidden_size, 1, noise_std, alpha, rank=2, n_supports=2, gaussian_basis_dim=7)
...
train(net, x_train, y_train, mask_train, lr=1e-5, n_epochs=5, keep_best=True, cuda=True,
      clip_gradient=1, resample=True)
...
dms.setup()
x_train, y_train, mask_train, x_val, y_val, mask_val = dms.generate_dms_data(1000)
train(net, x_train, y_train, mask_train, lr=1e-5, n_epochs=5, keep_best=True, cuda=True, resample=True)
loss, acc = dms.test_dms(net, x_val, y_val, mask_val)
print("final loss: {}\nfinal accuracy: {}".format(loss, acc))
dms.confusion_matrix(net)
