...
Change the value of rho for different figures
"""
...
for i in range(n_nets):
    net = FullRankRNN(input_size, hidden_size, 1, noise_std, alpha, train_wi=True, rho=.1)  ## Change rho for different figures
    w0 = net.wrec.detach().numpy().copy()
    train(net, x_train, y_train, mask_train, 100, lr=1e-4, clip_gradient=1,
          early_stop=0.05, keep_best=True, cuda=True)
    # net.load_state_dict(torch.load(f'../models/dms_many_fr2/{i}.pt', map_location='cpu'))
    loss, acc = dms.test_dms(net, x_val, y_val, mask_val)
    print("final loss: {}\nfinal accuracy: {}".format(loss, acc))

    # forget this network if training didn't succeed (even though we will have less than 100 networks in the end)
    if acc < 0.95:
        continue
    acc_vals.append(acc)
    loss_vals.append(loss)
    torch.save(net.state_dict(), f"../models/dms_many_fr2/{i}.pt")
    ...
    for j, rank in enumerate(ranks_trunc):
        ...
        for k in range(n_samples):
            ...
            loss, acc = dms.test_dms(net_reb, x_val, y_val, mask_val)
            accs_res[i, k, j] = acc
            losses_res[i, k, j] = loss


np.savez('../data/dms_fr_results2.npz', p_vals, c_vals, preg_vals, creg_vals, acc_vals, loss_vals, trunc_accs, trunc_losses,
         accs_res, losses_res)

np.save('../data/dms_fr_c_boot_distr2.npy', np.array(cdistr))
